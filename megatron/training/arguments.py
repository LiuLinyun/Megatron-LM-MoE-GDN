...
def _add_network_size_args(parser):
    exclude = [
        ...
        # already generated by _add_experimental_attention_variant_args
        ...
        "linear_num_shared_heads",
        "linear_num_routed_heads",
        "linear_write_topk",
        "linear_read_topk",
        "linear_write_coeff_for_read",
        "linear_moe_router_enable_expert_bias",
    ]
...
def _add_experimental_attention_variant_args(parser):
    ...
    group.add_argument('--experimental-attention-variant', type=str, default=None,
                       choices=['gated_delta_net', 'moe_gated_delta_net', 'dsa'],
                       help='Type of experimental attention variant to use.')
    # Dense GatedDeltaNet parameters
    group.add_argument('--linear-conv-kernel-dim', type=int, default=None,
                       help='Causal conv1d kernel size for GatedDeltaNet.')
    group.add_argument('--linear-key-head-dim', type=int, default=None,
                       help='Query/key head dimension for GatedDeltaNet.')
    group.add_argument('--linear-value-head-dim', type=int, default=None,
                       help='Value/gate head dimension for GatedDeltaNet.')
    group.add_argument('--linear-num-key-heads', type=int, default=None,
                       help='Number of query/key heads for GatedDeltaNet.')
    group.add_argument('--linear-num-value-heads', type=int, default=None,
                       help='Number of value/gate heads for GatedDeltaNet.')
    # MoE-GatedDeltaNet parameters
    group.add_argument('--linear-num-shared-heads', type=int, default=None,
                       help='Number of always-active shared heads for MoE-GatedDeltaNet.')
    group.add_argument('--linear-num-routed-heads', type=int, default=None,
                       help='Total routed-head pool size for MoE-GatedDeltaNet.')
    group.add_argument('--linear-write-topk', type=int, default=None,
                       help='Top-k heads selected to write state per token (MoE-GDN).')
    group.add_argument('--linear-read-topk', type=int, default=None,
                       help='Top-k heads selected to read state per token (MoE-GDN).')
    group.add_argument('--linear-write-coeff-for-read', type=float, default=0.5,
                       help='Mixing coefficient for write/read routing score blend (MoE-GDN).')
    group.add_argument('--linear-moe-router-enable-expert-bias', action='store_true',
                       help='Enable expert bias in MOE router (for GatedDeltaNet).')
    return parser
